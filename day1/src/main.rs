...
fn calculate_module_fuel(module_weight: i32) -> i32 {
    return round::floor((module_weight / 3).into(), 2) as i32 - 2;
}

fn calculate_module_fuel_rec(mut total: i32, module_weight: i32) -> i32 {
...
fn main() {
    ...
    for (_, line) in reader_part1.lines().enumerate() {
        ...
        let module_weight = line.parse::<i32>().unwrap();
